...
# Edited some detail based on Basab's code
...
# Rewrite the class DoubleConv, it contains now one cyclicpadding per layer
...
# Rewrite the class Down with only one Double-Convolution-Block in it
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = DoubleConv_new(in_channels, out_channels, kernel_size, in_channels // 2, residual=True)
            ...
            self.conv = DoubleConv_new(in_channels, out_channels, kernel_size)
